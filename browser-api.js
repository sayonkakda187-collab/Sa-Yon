(function bootstrapBrowserApi() {
  ...
  const PLATFORM_CONFIG = {
    pinterest: { label: "Pinterest", folderName: "Pinterest_Downloader", prefix: "Pinterest" },
    youtube: { label: "YouTube", folderName: "YouTube_Downloader", prefix: "YouTube" },
    tiktok: { label: "TikTok", folderName: "TikTok_Downloader", prefix: "TikTok" },
  };
  ...
  async function getDefaultFolder(platform = "pinterest") {
    await ensureRestored();
    if (downloadState.folderHandle) {
      return downloadState.folderLabel;
    }
    return `Downloads/${getPlatformConfig(platform).folderName}`;
  }

  async function getEnvironment(platform = "pinterest") {
    await ensureRestored();
    const defaultLabel = await getDefaultFolder(platform);
    ...
    return {
      ...
      saveHint: supportsFileSystemAccess
        ? downloadState.folderHandle
          ? "Files will be written directly into the selected folder. Installed browser mode keeps this flow available on supported desktop browsers."
          : `Choose a folder to enable direct writes. Without folder access, files fall back to browser-managed downloads. Recommended default: ${defaultLabel}.`
  ...
  function extractUrls(text, platform) {
    if (platform === "youtube") {
      return extractYouTubeUrls(text);
    }
    if (platform === "tiktok") {
      return extractTikTokUrls(text);
    }
    return extractPinterestUrls(text);
  }
  ...
  async function startDownloads(payload) {
    ...
    const items = Array.isArray(payload && payload.items)
      ? payload.items.filter((item) => item && item.selected && item.directUrl)
      : [];
  ...
  async function exportLog(payload) {
    const platform = payload && payload.platform ? payload.platform : "pinterest";
    const fileName = `${platform}-download-log-${new Date().toISOString().replace(/[:.]/g, "-")}.json`;
  ...
  async function saveDirectlyToFolder(items, settings) {
    ...
    for (const item of items) {
      ...
      try {
        ...
        const mediaUrl = buildMediaProxyUrl(item, duplicate.fileName);
  ...
  async function startBrowserManagedDownloads(items, settings) {
    ...
    for (const item of items) {
      try {
        ...
        const fileName = buildFileName(item, settings);
        const mediaUrl = buildMediaProxyUrl(item, fileName);

        emit({
          type: "item-status",
          itemId: item.id,
          status: "Downloading",
          totalBytes: item.fileSize || null,
          summary: withSpeed(summary, startedAt, 0),
        });
  ...
  async function resolveTargetDirectory(baseHandle, item, settings) {
    ...
    if (settings.subfolderByPlatform && item.platformLabel) {
      segments.push(sanitizeFilename(item.platformLabel, item.platformLabel));
    }
    if ((settings.subfolderByCreator || settings.subfolderByBoard) && (item.creatorName || item.channelName || item.boardName)) {
      segments.push(sanitizeFilename(item.creatorName || item.channelName || item.boardName, "creator"));
    }
    if ((settings.subfolderByContentType || settings.subfolderByMediaType) && (item.contentTypeLabel || item.mediaType)) {
      segments.push(sanitizeFilename(item.contentTypeLabel || item.mediaType, "content"));
    }
  ...
  function buildMediaProxyUrl(item, fileName) {
    const params = new URLSearchParams({
      url: item.directUrl,
      filename: fileName,
      platform: item.platform || "pinterest",
      sourceUrl: item.normalizedUrl || item.sourceUrl || item.directUrl,
      formatId: item.selectedFormatId || item.selectedVariantKey || "",
    });

    if (item.requestHeaders && Object.keys(item.requestHeaders).length) {
      params.set("headers", encodeHeaderPayload(item.requestHeaders));
    }

    return `/api/media?${params.toString()}`;
  }

  function encodeHeaderPayload(headers) {
    return toBase64Url(JSON.stringify(headers));
  }

  function buildFileName(item, settings) {
    const config = getPlatformConfig(item.platform);
    const prefix = sanitizeFilename(settings.customPrefix || config.prefix, config.prefix);
    const parts = [prefix];

    if (settings.useCreator && (item.creatorName || item.channelName)) {
      parts.push(sanitizeFilename(item.creatorName || item.channelName, "creator"));
    }

    if ((settings.useTitle || settings.usePinTitle) && (item.title || item.pinTitle)) {
      parts.push(sanitizeFilename(item.title || item.pinTitle, "item"));
    }

    parts.push(item.resourceId || item.pinId || "unknown");
    parts.push((item.contentTypeLabel || item.mediaType || "media").replace(/\s+/g, "_"));
  ...
  function normalizeYouTubeUrl(rawUrl) {
    try {
      ...
      } else if (/\/shorts\//i.test(parsed.pathname) || /\/embed\//i.test(parsed.pathname) || /\/live\//i.test(parsed.pathname)) {
      ...
      return { ok: true, url: /^\/shorts\//i.test(parsed.pathname) ? `https://www.youtube.com/shorts/${videoId}` : `https://www.youtube.com/watch?v=${videoId}` };
    } catch {
      return { ok: false };
    }
  }
