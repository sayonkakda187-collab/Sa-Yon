...
const { downloadToFile } = require("./yt-dlp-client");
const {
  formatDateStamp,
...
async function downloadItems(payload = {}, emit = () => {}) {
  ...
  const firstPlatform = Array.isArray(payload.items) && payload.items[0] ? payload.items[0].platform : "pinterest";
  const outputFolder = payload.saveFolder || getDefaultDownloadFolder(firstPlatform);
    ...
    while (true) {
      ...
      try {
        ...
        if (item.platform === "tiktok") {
          emit({ type: "item-status", itemId: item.id, status: "Downloading", totalBytes: item.fileSize || null, summary: withSpeed(summary, startedAt, downloadedBytes) });
          await downloadToFile({
            sourceUrl: item.normalizedUrl || item.sourceUrl,
            formatId: item.selectedFormatId || item.selectedVariantKey,
            outputPath: duplicateResult.filePath,
          });
          const stat = await fs.stat(duplicateResult.filePath);
          downloadedBytes += stat.size;
          summary.completed += 1;
          summary.remaining -= 1;
          const logEntry = { sourceUrl: item.sourceUrl, directUrl: item.directUrl, savedPath: duplicateResult.filePath, status: "Saved", bytesDownloaded: stat.size, error: null };
          log.push(logEntry);
          emit({
            type: "item-status",
            itemId: item.id,
...
async function resolveTargetDirectory(baseFolder, item, settings) {
  ...
  if ((settings.subfolderByCreator || settings.subfolderByBoard) && (item.creatorName || item.boardName || item.channelName)) {
    segments.push(sanitizeFilename(item.creatorName || item.boardName || item.channelName, "creator"));
  }
  if ((settings.subfolderByContentType || settings.subfolderByMediaType) && (item.contentTypeLabel || item.mediaType)) {
    segments.push(sanitizeFilename(item.contentTypeLabel || item.mediaType, "media"));
  }
  return path.join(baseFolder, ...segments);
}

function buildFileName(item, settings) {
  const defaultPrefix = getPlatformConfig(item.platform).prefix;
  const prefix = sanitizeFilename(settings.customPrefix || defaultPrefix, defaultPrefix);
  const parts = [prefix];
  ...
  if ((settings.usePinTitle || settings.useTitle) && (item.title || item.pinTitle)) {
