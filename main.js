if (!process.versions.electron) {
  ...
  const { analyzeUrls } = require("./src/platform-service");
  ...
  ipcMain.handle("folder:get-default", async (_event, platform = "pinterest") => {
    const folderPath = getDefaultDownloadFolder(platform);
  ...
  ipcMain.handle("media:analyze", async (_event, payload = {}) => {
    ...
    return analyzeUrls(platform, urls);
  });
  ...
  ipcMain.handle("log:export", async (_event, payload = {}) => {
    const defaultName = `downloader-log-${new Date().toISOString().replace(/[:.]/g, "-")}.json`;
    const result = await dialog.showSaveDialog({
      title: "Export download log",
      defaultPath: path.join(getDefaultDownloadFolder(payload.platform || "pinterest"), defaultName),
