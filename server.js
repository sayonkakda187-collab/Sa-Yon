...
const { analyzeUrls } = require(resolveModule("./src/platform-service", "./platform-service"));
const { downloadToFile } = require(resolveModule("./src/yt-dlp-client", "./yt-dlp-client"));
...
server.listen(PORT, HOST, () => {
  console.log(`Downloader Hub PWA running at http://${HOST}:${PORT}`);
  console.log("For installable production use, serve the same app over HTTPS.");
});

async function handleAnalyze(request, response) {
  const payload = await readJsonBody(request);
  const platform = String(payload.platform || "pinterest").toLowerCase();
  const urls = Array.isArray(payload.urls) ? payload.urls : [];
  const result = await analyzeUrls(platform, urls);
...
async function handleMediaProxy(requestUrl, method, response) {
  ...
  const requestHeaders = decodeProxyHeaders(requestUrl.searchParams.get("headers"));

  if (platform === "tiktok") {
    await handleYtDlpProxy(requestUrl, method, response, filename);
    return;
  }
...
async function handleYtDlpProxy(requestUrl, method, response, filename) {
  const sourceUrl = requestUrl.searchParams.get("sourceUrl");
  const formatId = requestUrl.searchParams.get("formatId");

  if (!sourceUrl) {
    respondJson(response, 400, { error: "missing_source_url" });
    return;
  }

  const extension = path.extname(filename || "") || ".mp4";
  const tempDir = path.join(ROOT_DIR, ".cache", "proxy-downloads");
  const tempPath = path.join(tempDir, `proxy-${Date.now()}-${Math.random().toString(36).slice(2)}${extension}`);

  await fsPromises.mkdir(tempDir, { recursive: true });

  try {
    await downloadToFile({ sourceUrl, formatId, outputPath: tempPath });
    const stat = await fsPromises.stat(tempPath);
    response.statusCode = 200;
    response.setHeader("Content-Type", "video/mp4");
    response.setHeader("Content-Length", stat.size);
    response.setHeader("Content-Disposition", buildContentDisposition(filename || path.basename(tempPath)));
    response.setHeader("Cache-Control", "no-store");
    ...
    await pipeline(fs.createReadStream(tempPath), response);
  } finally {
...
function decodeProxyHeaders(rawHeaders) {
  if (!rawHeaders) {
    return {};
  }

  try {
    const parsed = JSON.parse(Buffer.from(rawHeaders, "base64url").toString("utf8"));
    const allowedHeaders = ["accept", "accept-language", "cookie", "origin", "range", "referer", "user-agent"];

    return Object.fromEntries(
      Object.entries(parsed).filter(([key, value]) => allowedHeaders.includes(String(key).toLowerCase()) && value),
    );
  } catch {
    return {};
  }
